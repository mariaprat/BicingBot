...
def geometric_graph(d, bicing):
    ...
    # nodes in the same cell of the grid or in adjacent cells (without
    # repetitions).
...
def build_flow_graph(requiredBikes, requiredDocks, G, bicing, bikes):
    # We create the flow graph F from G. Each station is represented by three
    # nodes the station itself is a node named g + index. The nodes sg + index
    # represent the amount of bikes that the station has to give to other
    # stations, and the nodes tg + index represent the amount of bikes that the
    # station has to receive.
...
# the minimum flow and the edge with the highest cost with its information.
def update(G, bikes):
    ...
    # We try doing the minimum flow using simplex function in NetworkX.
    try:
...
def true_route(addresses, G, position, bikes):
    ...
    # Only stations with bikes are connected to the start (that is stations
    # with numbers 2 or 3). And only stations with docks are conected to the
    # end (that is stations with numbers 1 or 3).
    for a in F.nodes():
